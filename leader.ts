...
export class Leader {
    ...
    applyOnce() {
        ...
        const ret = this._sendMessage(this, 'apply') // send out that this one is applying
            .then(() => sleep(this._options.responseTime)) // let others time to respond
            .then(() => {
                if (stopCriteria) return Promise.reject(new Error());
                else return this._sendMessage(this, 'apply');
            })
            .then(() => sleep(this._options.responseTime)) // let others time to respond
            .then(() => {
                if (stopCriteria) return Promise.reject(new Error());
                else return this._sendMessage(this);
            })
            .then(() => this.beLeader(this)) // no one disagreed -> this one is now leader
            .then(() => true)
    ...
    awaitLeadership() {
        if (
            ...
            this._aLP = this._awaitLeadershipOnce(this);
        }
        return this._aLP;
    }
    ...
    die() {
        ...
        return this._sendMessage(this, 'death');
    }


    _awaitLeadershipOnce(leaderElector) {
     ...
     */
    _sendMessage(leaderElector, action?) {
        const msgJson = {
            context: 'leader',
            action,
    ...
    beLeader(leaderElector) {
        ...
        const isLeaderListener = msg => {
            if (msg.context === 'leader' && msg.action === 'apply') {
                this._sendMessage(leaderElector, 'tell');
            }
            ...
            if (msg.context === 'leader' && msg.action === 'tell' && !leaderElector._dpLC) {
                ...
                this._sendMessage(leaderElector, 'tell'); // ensure other leader also knows the problem
            }
        };
        leaderElector._channel.addEventListener('internal', isLeaderListener);
        leaderElector._lstns.push(isLeaderListener);
        return this._sendMessage(leaderElector, 'tell');
    }
}

export function fillOptionsWithDefaults(options, channel) {
    if (!options) options = {};
...
export function createLeaderElection(channel, options?) {
    ...
    const elector = new Leader(channel, options);
